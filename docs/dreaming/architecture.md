...
flowchart TB
    ...
    subgraph Dispatch["dispatch.py — fast, &lt;1s, never blocks"]
        RG{"Recursion guard:<br/>CLAUDE_DREAMING / CHILD_SESSION?"}
    ...
    subgraph State["Durable state (~/.claude/dreaming)"]
        SPOOL[("spool/*.json")]
...
1. `--bare` makes the nested run skip hook/skill/memory auto-discovery (so the dreaming hooks never
   load inside it).
2. The `CLAUDE_DREAMING=1` sentinel is set on the worker and its children; `dispatch.py` exits
...
Types are `user`, `feedback`, `project`, `reference`. See `hooks/dreaming/prompts/dream.system.md`
