...
auto-discovery (so this nested run does not itself trigger dreaming), and the
``CLAUDE_DREAMING=1`` sentinel is a second recursion guard.
...
HERE = Path(__file__).resolve().parent.parent  # hooks/dreaming
