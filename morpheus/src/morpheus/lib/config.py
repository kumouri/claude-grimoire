...
def config_path() -> Path:
    env = os.environ.get("CLAUDE_MORPHEUS_CONFIG")
    return Path(env) if env else morpheus_home() / "config.json"
...
def load() -> dict:
    ...
    mode = os.environ.get("CLAUDE_MORPHEUS_MODE")
    if mode:
        cfg["mode"] = mode
    if os.environ.get("CLAUDE_MORPHEUS_DISABLED") == "1":
        cfg["enabled"] = False
    return cfg
